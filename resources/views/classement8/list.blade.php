    ...
    <li class="list-group-item d-flex align-items-center" style="border-radius:5px;">
      <div class="col-lg-10">
       ...
       <img src="{{$path}}" width="50" height="50"/>
        ...
        <strong></strong>
        <?php
        $img=$classement8->equipe->idComposante;
        $pathimg;
        if($img==1){
      ...
      elseif($img==3){
        ...
        <strong>{{$classement8->equipe->NomEquipe}}</strong>
        @if(strlen($classement8->equipe->Slogan) > 50)
          {{substr($classement8->equipe->Slogan, 0, 50)}}...
        @else
          {{$classement8->equipe->Slogan}}
        @endif
      </div>          
      <div class="col text-end">
        <a href="{{route('classement8.show', $classement8->id)}}" class="stretched-link"></a>
      </div>
      &nbsp;
      </div>
      @auth
...
<!--
<button type="submit" formaction="{{route('classement1.destroy', $classement8->id)}}" form="deleteForm" class="btn btn-sm btn-danger mb-1">
