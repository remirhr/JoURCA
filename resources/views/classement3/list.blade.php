    ...
    <li class="list-group-item d-flex align-items-center" style="border-radius:5px;">
      <div class="col-lg-10">
       ...
       <img src="{{$path}}" width="50" height="50"/>
        ...
        <strong></strong>
        <?php
        $img=$classement3->equipe->idComposante;
        $pathimg;
        if($img==1){
      ...
      elseif($img==3){
        ...
        <strong>{{$classement3->equipe->NomEquipe}}</strong>
        @if(strlen($classement3->equipe->Slogan) > 50)
          {{substr($classement3->equipe->Slogan, 0, 50)}}...
        @else
          {{$classement3->equipe->Slogan}}
        @endif
      </div>        
      <div class="col text-end">
        <a href="{{route('classement3.show', $classement3->id)}}" class="stretched-link"></a>
      </div>
      &nbsp;
      </div>
      @auth
