    ...
    <li class="list-group-item d-flex align-items-center" style="border-radius:5px;">
      <div class="col-lg-10">
       ...
       <img src="{{$path}}" width="50" height="50"/>
        ...
        <strong></strong>
        <?php
        $img=$classement15->equipe->idComposante;
        $pathimg;
        if($img==1){
      ...
      elseif($img==3){
        ...
        <strong>{{$classement15->equipe->NomEquipe}}</strong>
        @if(strlen($classement15->equipe->Slogan) > 50)
          {{substr($classement15->equipe->Slogan, 0, 50)}}...
        @else
          {{$classement15->equipe->Slogan}}
        @endif
      </div>      
