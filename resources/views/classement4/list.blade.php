    ...
    <li class="list-group-item d-flex align-items-center" style="border-radius:5px;">
      <div class="col-lg-10">
       ...
       <img src="{{$path}}" width="50" height="50"/>
        ...
        <strong></strong>
        <?php
        $img=$classement4->equipe->idComposante;
        $pathimg;
        if($img==1){
      ...
      elseif($img==3){
        ...
        <strong>{{$classement4->equipe->NomEquipe}}</strong>
        @if(strlen($classement4->equipe->Slogan) > 50)
          {{substr($classement4->equipe->Slogan, 0, 50)}}...
        @else
          {{$classement4->equipe->Slogan}}
        @endif
      </div>       
      <div class="col text-end">
        <a href="{{route('classement4.show', $classement4->id)}}" class="stretched-link"></a>
      </div>
      &nbsp;
      </div>
      @auth
