    ...
    <li class="list-group-item d-flex align-items-center" style="border-radius:5px;">
      <div class="col-lg-10">
       ...
       <img src="{{$path}}" width="50" height="50"/>
        ...
        <strong></strong>
        <?php
        $img=$classement6->equipe->idComposante;
        $pathimg;
        if($img==1){
      ...
      elseif($img==3){
        ...
        <strong>{{$classement6->equipe->NomEquipe}}</strong>
        @if(strlen($classement6->equipe->Slogan) > 50)
          {{substr($classement6->equipe->Slogan, 0, 50)}}...
        @else
          {{$classement6->equipe->Slogan}}
        @endif
      </div>         
      <div class="col text-end">
        <a href="{{route('classement6.show', $classement6->id)}}" class="stretched-link"></a>
      </div>
      &nbsp;
      </div>
      @auth
