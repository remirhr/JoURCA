    ...
    <li class="list-group-item d-flex align-items-center" style="border-radius:5px;">
      <div class="col-lg-10">
       ...
       <img src="{{$path}}" width="50" height="50"/>
        ...
        <strong></strong>
        <?php
        $img=$classement9->equipe->idComposante;
        $pathimg;
        if($img==1){
      ...
      elseif($img==3){
        ...
        <strong>{{$classement9->equipe->NomEquipe}}</strong>
        @if(strlen($classement9->equipe->Slogan) > 50)
          {{substr($classement9->equipe->Slogan, 0, 50)}}...
        @else
...
<!--
<button type="submit" formaction="{{route('classement1.destroy', $classement9->id)}}" form="deleteForm" class="btn btn-sm btn-danger mb-1">
